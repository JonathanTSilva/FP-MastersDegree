...
def detect_opcua_attack(pkt):
    global opcua_request_count
    try:
        if hasattr(pkt, 'opcua') and pkt.opcua.request_id:
            opcua_request_count += 1
    except:
        pass
...
try:		
    ...
    print(f'This is the type of cap variable: {type(cap)}')
    ...
    for pkt in cap:
        print(f'This is the type of cap variable: {pkt}')
        #neglect eth layer packet
        if not hasattr(pkt, 'ip'):
            continue
        src_ip=find_src_ip(pkt)
        dst_ip=find_dst_ip(pkt)
        detect_ftp_attack(pkt)
        if opcua_request_count > 90:
            print("Potential OPC UA attack detected!")
