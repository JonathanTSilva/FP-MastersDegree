...
Este capítulo apresenta os principais aspectos da bancada experimental utilizada, descreve as estratégias adotadas para sua implementação e desenvolvimento, e detalha os diferentes cenários de ataques cibernéticos simulados, bem como o delineamento da abordagem empregada na condução dos testes, de modo a garantir a robustez e a validade dos resultados obtidos frente aos objetivos inicialmente propostos.
...
\section{Ataques Cibernéticos em Redes Industriais OPC UA} \label{sec:attacks}
    ...
    \subsection{\textit{Denial of Service (DoS)}}
        ...
        Diversos cenários podem ser explorados para a execução de um ataque de negação de serviço. Além do impacto causado pela inundação da rede, destaca-se a sobrecarga no processamento do servidor, especialmente em ataques intensivos, nos quais é precisa validar as certificações para responder às solicitações. Segundo \citeonline{neu2019}, os principais cenários desse tipo de ataque às redes OPC UA incluem:
...
<esquema>://<endereço>:<porta>/<DiscoveryEndpoint>
    ...
    \subsection{Análise dos Ataques e Vulnerabilidades}\label{sec:analise}
        ...
        A partir da análise dos gráficos gerados e do comportamento da rede e dos dispositivos durante e após os ataques, torna-se possível identificar e classificar as vulnerabilidades conforme as categorias mencionadas. Além disso, avalia-se a eficácia das diferentes políticas de segurança do OPC UA (`None', `Sign' e `Sign\&Encrypt'). A interpretação dos resultados obtidos é fundamental para detectar falhas potenciais e propor medidas corretivas e preventivas, visando a mitigação dos riscos de segurança nessas redes.
