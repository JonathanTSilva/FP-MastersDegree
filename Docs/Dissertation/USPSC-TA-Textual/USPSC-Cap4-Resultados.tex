...
\section{Propostas de Melhorias e Mitigações} \label{sec:melhorias-mitigacoes}
    ...
    \subsection{Recomendações para Comunicações Seguras com o Protocolo OPC UA}
        ...
        Em primeiro lugar, assim como em qualquer outro \textit{software} ou protocolo, a atualização regular é fundamental para garantir a segurança do OPC UA. De modo geral, atualizações incluem correções de \textit{bugs} e aplicações de \textit{patches} de segurança projetados para mitigar vulnerabilidades conhecidas. Portanto, recomenda-se que todo o IACS seja mantido atualizado, abrangendo sistemas operacionais, aplicativos, bibliotecas e protocolos, incluindo o OPC UA.
    ...
    \subsection{Melhorias na Infraestrutura e Gestão de Redes}
        ...
        Para assegurar a segurança e a resiliência das redes industriais OPC UA, torna-se primordial a adoção de estratégias de mitigação que englobem tanto a infraestrutura quanto a gestão da rede. Dada a complexidade e a criticidade inerentes aos sistemas de automação e controle industrial, uma abordagem de proteção robusta e abrangente contra ciberataques, como os explorados neste trabalho, é indispensável. A seguir, serão apresentadas práticas e tecnologias cruciais para o fortalecimento da infraestrutura de rede, visando prevenir a interceptação e a manipulação do tráfego, bem como a implementação de mecanismos de monitoramento eficazes para a detecção e resposta ágil a atividades suspeitas.
        
        Inicialmente, a mitigação de ataques MITM, que incluem o roubo de portas e a falsificação da tabela ARP, pode ser significativamente aprimorada por meio de melhorias na infraestrutura do IACS. A segurança de porta nos \textit{switches} emerge como uma medida fundamental para impedir que dispositivos não autorizados interceptem o tráfego, restringindo o número de endereços MAC aprendidos por porta e desativando automaticamente aquelas que apresentem violações. Adicionalmente, a configuração de entradas estáticas na tabela ARP impede que equipamentos maliciosos manipulem esses dados para desviar o tráfego, representando uma tática eficaz em redes com topologia e endereços IP bem definidos.
        
        A robustez da segurança em infraestruturas de rede perpassa, adicionalmente, pela implementação criteriosa de mecanismos de monitoramento, tanto ativo quanto passivo, os quais assumem um papel crucial na detecção e subsequente resposta a atividades anômalas. Ferramentas como \texttt{arpwatch} e \texttt{ettercap} permitem a identificação de alterações na tabela ARP e a interceptação de tráfego malicioso em tempo real. Dentre estas, o monitoramento revela-se particularmente indispensável na identificação e mitigação de ataques do tipo MITM em curso, preservando assim a integridade e a confidencialidade dos fluxos de dados. A combinação dessas técnicas de monitoramento com a implementação de sistemas de detecção de instrusões fortalece a postura de segurança da rede, permitindo a identificação precoce de ameaças e a resposta rápida a incidentes.
        
        Paralelamente às estratégias de detecção, a criptografia de dados em trânsito, mediante a aplicação de protocolos criptográficos seguros como TLS/SSL, configura-se igualmente como um pilar essencial para proteger informações sensíveis contra acessos não autorizados. Mesmo que um invasor consiga capturar o tráfego por meio de técnicas de \textit{sniffing}, a criptografia garante que os dados capturados permaneçam ilegíveis e inoperantes sem a chave de decriptação correspondente. A eficácia desta abordagem é corroborada, por exemplo, nos ataques simulados neste trabalho, cujas redes OPC UA configuradas com o modo de segurança `Sign\&Encrypt' demonstrou alta capacidade de neutralizar a exploração da informação.

        Complementarmente a estas camadas de proteção, a limitação da superfície de ataque e o controle granular do tráfego de rede são eficazmente alcançados por meio da segmentação da rede, uma prática usualmente implementada mediante a utilização de VLANs (do inglês \textit{Virtual Local Area Network}). Ao segregar a infraestrutura de rede em zonas distintas e logicamente isoladas, a comunicação entre dispositivos circunscreve-se estritamente aos segmentos autorizados e necessários para suas funções, o que mitiga expressivamente o risco de acessos indevidos e a propagação de comprometimentos. Essa abordagem não apenas obstaculiza a movimentação lateral de ameaças cibernéticas, como também minimiza o impacto de eventuais incidentes de segurança, confinando-os a porções delimitadas da rede.
        
        Nesse contexto, conforme assinalado por \citeonline{knapp2024}, o estabelecimento da segurança de rede para a proteção de zonas definidas impõe a implementação de um rigoroso código de conduta. As normativas de segurança devem harmonizar-se com os canais de comunicação intrínsecos a esses perímetros controlados. Os controles de segurança de rede são projetados tanto para impedir acessos não autorizados aos sistemas internos quanto para evitar que esses sistemas se conectem a redes externas de maneira descontrolada. Destarte, para assegurar a proteção efetiva do tráfego de entrada e saída da rede, preconiza-se a adoção das seguintes medidas:
        ...
        A microsegmentação surge como outra estratégia valiosa, permitindo um controle mais preciso sobre a comunicação entre dispositivos, aplicações e usuários, facilitando a aplicação de políticas de segurança específicas para cada segmento da rede. Além disso, a distinção entre detecção e prevenção é fundamental, especialmente em redes industriais onde a disponibilidade e o desempenho são prioritários. Nessas redes, o IDS é preferencialmente utilizado dentro das zonas industriais, evitando a perda de pacotes devido a falsos positivos, enquanto o IPS é mais adequado para os limites entre zonas industriais e empresariais, onde o impacto de interrupções pode ser melhor gerenciado.

        Ademais, a segurança dos dispositivos dentro dessas zonas é igualmente crítica, independentemente de sua conexão à rede. Equipamentos que não podem ser protegidos por métodos tradicionais devem ser isolados em subzonas de segurança específicas, com condutos rigorosamente controlados. Mesmo dispositivos embarcados, frequentemente vulneráveis, devem ser protegidos sempre que possível, através de controles de segurança compensatórios nos condutos que os interligam às zonas mais amplas, garantindo a segurança em todos os níveis da infraestrutura.

        Em suma, a proteção de dispositivos e aplicações dentro das zonas exige não apenas controle de acesso rigoroso, mas também um monitoramento contínuo das comunicações. Os dispositivos presentes nessas zonas, especialmente aqueles com interfaces de rede, devem ser protegidos por mecanismos robustos de autenticação, controle de acesso e manutenção adequada. O monitoramento das atividades dos dispositivos, facilitado pela implementação de um SIEM, permite a consolidação e análise centralizada de logs de segurança, viabilizando a identificação de padrões de tráfego suspeitos e a resposta rápida a incidentes de segurança, consolidando assim uma postura de segurança robusta e abrangente para a infraestrutura e gestão de redes industriais OPC UA.

    \subsection{Normas e Regulamentos}

        No cenário da segurança cibernética, um espectro de normas, diretrizes e regulamentos é estabelecido por entidades governamentais e industriais. Estes variam desde as chamadas ``melhores práticas'' até exigências mandatórias, cujo descumprimento pode acarretar sanções legais e financeiras. Embora muitos desses documentos normativos abordem a segurança da informação de maneira geral, observa-se um crescimento significativo no número de publicações dedicadas especificamente aos sistemas de automação e controle industrial (IACS). Conforme elucida \citeonline{knapp2024}, nos Estados Unidos, destacam-se os Padrões de Confiabilidade de Proteção de Infraestruturas Críticas (CIP) da NERC, os Padrões Antiterrorismo para Instalações Químicas (CFATS) do DHS, a Regulamentação de Segurança de Instalações Nucleares da NRC e as recomendações do NIST na Publicação Especial 800-82. Na Europa, sobressaem o EU M/490 e o SGCG para sistemas de energia, além de diversas publicações da ENISA. Em âmbito global, a série ISO/IEC 27000, com ênfase na ISO-27002:2013 (``Código de prática para controles de segurança da informação''), possui ampla adoção.
        
        Entre os padrões de maior relevância para a segurança em ambientes industriais, a norma ISA/IEC 62443 (anteriormente ISA 99), desenvolvida pela Sociedade Internacional de Automação (ISA, do inglês \textit{International Society of Automation}) em parceria com a Comissão Eletrotécnica Internacional (IEC, do inglês \textit{International Electrotechnical Commission}), assume um papel central. Este padrão dedica-se à segurança de sistemas de automação e controle industrial, sendo aplicável a qualquer organização ou setor que os utilize. Os níveis de segurança definidos na norma visam proteger a confidencialidade das informações, estratificando-os de acordo com a capacidade e os recursos de potenciais invasores.
